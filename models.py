...
class NearEarthObject:
    ...
    def __init__(self, **info):
        ...
        self.diameter = float(
            'nan') if not info['diameter'] else float(info['diameter'])
    ...
    def __str__(self):
        ...
        string = 'NEO '
        if self.fullname:
            string += f'{self.fullname}'
        string += ' has a diameter of '
        if self.diameter:
            string += f'{self.diameter} km and '
        if self.hazardous:
    ...
    def writer(self):
        """Return a dictionary for name, diameter and hazardous"""
        return {'name': self.name, 'diameter_km': self.diameter, 'potentially_hazardous': self.hazardous}
...
class CloseApproach:
    ...
    def __init__(self, **info):
        ...
        self.distance = float(
            info['distance']) if info['distance'] else float('nan')
        self.velocity = float(
            info['velocity']) if info['velocity'] else float('nan')
    ...
    def writer_json(self):
        """Return a dictionary of variables for json type"""
        return {'datetime_utc': self.time_str, 'distance_au': self.distance, 'velocity_km_s': self.velocity,
                'neo': {'name': self.neo.name, 'designation': self._designation, 'diameter_km': self.neo.diameter,
                        'potentially_hazardous': self.neo.hazardous}}

    def writer_csv(self):
        """Return a list of variables for csv type"""
        return [self.time_str, self.distance, self.velocity, self._designation,
                self.neo.name, self.neo.diameter, self.neo.hazardous]
